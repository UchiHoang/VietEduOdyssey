...
const DragDropGameComponent = ({ items, slots, onComplete, title }: DragDropGameProps) => {
  ...
  useEffect(() => {
    setShuffledItems(fisherYatesShuffle(items));
  }, [items]);
  ...
  const handleDrop = (slotId: string) => {
    ...
    if (draggedItem) {
      setDroppedItems(prev => ({ ...prev, [slotId]: draggedItem }));
      setDraggedItem(null);
    }
  };
  ...
  const isItemPlaced = (itemId: string) => Object.values(droppedItems).includes(itemId);
  ...
  return (
    <div className="w-full max-w-5xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="text-center text-sm text-muted-foreground mb-4">
        {showResults ? t.game.dragResult : t.game.dragInstruction}
      </div>
        ...
        {slots.map((slot) => {
          ...
          return (
            ...
            >
              ...
              {slot.image && <img src={slot.image} alt={slot.label} className="w-16 h-16 mx-auto mb-2 object-contain opacity-30" />}

              {itemId && item && (
                <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center gap-2">
                  {item.image && <img src={item.image} alt="" className="w-12 h-12 object-contain" />}
                  <span className="text-sm font-semibold text-center">{item.content}</span>
                  {!showResults && (
                    <button onClick={() => handleRemove(slot.id)} className="absolute top-2 right-2 p-1 rounded-full bg-background hover:bg-destructive hover:text-destructive-foreground transition-colors">
                      <X className="w-4 h-4" />
                    </button>
                  )}
                  {showResults && (
                    <div className="absolute top-2 right-2">
                      {isCorrect ? <CheckCircle2 className="w-6 h-6 text-green-600" /> : <X className="w-6 h-6 text-red-600" />}
                    </div>
                  )}
                </motion.div>
              )}
              ...
              {showResults && isCorrect === false && (
                <div className="mt-2 pt-2 border-t border-red-300 text-xs text-red-700">
                  ✅ {t.game.correctAnswerIs}: <strong>{getCorrectItemForSlot(slot.id)}</strong>
                </div>
              )}
            </div>
          );
      ...
      {!showResults && (
        <div className="space-y-4">
          <div className="text-sm font-medium text-center">{t.game.dragFrom}</div>
      ...
      <div className="text-center space-y-3">
        <div className="text-sm font-medium">
          {t.game.placed}: {Object.keys(droppedItems).length} / {items.length}
        </div>
        ...
        {showResults && (
          <div className="space-y-3">
            <div className={`p-4 rounded-lg animate-fade-in ${allCorrect ? "bg-green-100 text-green-800" : "bg-orange-100 text-orange-800"}`}>
              <p className="font-semibold">
                {allCorrect ? `🎉 ${t.game.allCorrect}` : `💡 ${t.game.reviewCorrect}`}
              </p>
            </div>
